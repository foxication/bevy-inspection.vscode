...
test('components synchronization', async (t: TestContext) => {
  ...
  await t.test('on working server', async (t: TestContext) => {
    ...
    t.assert.deepStrictEqual(
      componentErrors,
      JSON.parse(
        '{"server_all_types::UnregisteredType":{\
        "code":-23402,"message":"Unknown component type: `server_all_types::UnregisteredType`"\
        }}'
      )
    );
    ...
    // | ENUM               server_all_types::GameDifficulty --> server_all_types::GameDifficulty = Hard\n\
    // | | STRUCT           Hard --> server_all_types::GameDifficulty::Hard:\n\
    // | | | TUPLE          enemies --> i32 = 4\n\
    ...
    // | ENUM               server_all_types::WindowMode --> server_all_types::WindowMode = Window\n\
    // | | VALUE+SERDE      Window --> u32 = [512,256]\n';
